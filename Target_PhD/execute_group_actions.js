...
function executeGroupActions_(processAll) {
    ...
    if (processAll) {
        ...
        if (name === "Архів" || name === "Відповіді форми (1)") {
            ui.alert(`❌ Cannot execute group actions on "${name}". Please select the PhD sheet.`);
            return;
        }
    }
    ...
    for (const sheet of sheets) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const groupName = String(rowData[5 - 1] || "").trim(); // Col E (5) Group
            const recoveryEmail = String(rowData[9 - 1] || "").trim(); // Col I (9)
            ...
            try {
                if (action === "Send to Archive") {
                    ...
                    if (!mainEmail || !mainEmail.includes("@")) throw new Error("Missing or invalid gen email (Col F)");
                    if (!groupName) throw new Error("Missing group name (Col E) to determine restore path");
                    ...
                    const backupEmail = recoveryEmail;
                ...
                stats.errors.push(`Row ${rowIndex}: action "${action}" - ${e.message}`);
            }
        }
...
/**
 * Setup columns B and C on the PhD sheet.
 */
