const GROUP_ACTIONS_CFG = {
    ...
    START_ROW: 2,
    TIMESTAMP_COL: 20, // T
    ACTIONS: ["IDLE", "Send to Archive", "Move to PhD", "Move to Custom OU", "Notify Deletion", "Notify Alumni", "Delete Account", "Restore", "Change Main Email", "Change Name"],
    COLORS: {
        "IDLE": "#fff9c4",
...
function getRestoreOuPath_(groupRaw) {
    ...
    return `/2. Факультети/ФБМІ/${dept}/2. Магістратура/${gUpper}`;
}
...
function executeGroupActions_(processAll) {
    ...
    if (processAll) {
        ...
        if (name === "Архів" || name === "До АСПІРАНТУРИ" || name === "Відповіді форми (1)") {
            ui.alert(`❌ Cannot execute group actions on "${name}". Please select a group sheet.`);
            return;
        }
    }

    let stats = { archive: 0, phd: 0, customOu: 0, notified: 0, deleted: 0, restored: 0, emailChanges: 0, nameChanges: 0, errors: [] };
    ...
    for (const sheet of sheets) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const isPhd = recordType.includes("phd");
            const baseType = isPhd ? "phd" : "to_archiv";
            ...
            try {
                if (action === "Send to Archive") {
                ...
                } else if (action === "Move to PhD") {
                    if (!mainEmail || !mainEmail.includes("@")) throw new Error("Missing or invalid gen email (Col F)");

                    const userObj = typeof callWithRetry_ !== "undefined" ? callWithRetry_(() => AdminDirectory.Users.get(mainEmail), 5) : AdminDirectory.Users.get(mainEmail);
                    const currentOu = userObj.orgUnitPath;
                    const newOu = currentOu.replace('/2. Магістратура', '/3. Аспірантура');
                    ...
                    sheet.getRange(rowIndex, GROUP_ACTIONS_CFG.RECORD_TYPE_COL).setValue(`phd (In OU)`);
                    sheet.getRange(rowIndex, GROUP_ACTIONS_CFG.ACTION_COL).setValue("IDLE");
                    stats.phd++;
                    ...
                    sheet.getRange(rowIndex, 6).setValue(newEmail);
    ...
    if (stats.archive === 0 && stats.phd === 0 && stats.customOu === 0 && stats.notified === 0 && stats.deleted === 0 && stats.restored === 0 && stats.emailChanges === 0 && stats.nameChanges === 0 && stats.errors.length === 0) {
...
/**
 * Re-apply actions dropdown and formatting on all group sheets.
 * Run after deploying script changes that add new actions.
 */
function refreshGroupActionsFormattingAllSheets() {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    SpreadsheetApp.getUi().alert(`✅ Оновлено список actions на ${sheets.length} аркушах.`);
}
...
function setupGroupActionsFormatting_(sheet) {
    ...
    if (maxRows < GROUP_ACTIONS_CFG.START_ROW) return;
